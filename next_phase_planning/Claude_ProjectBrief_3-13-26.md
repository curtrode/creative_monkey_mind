...
This multi-model dialogue concept represents a clear, ambitious, achievable scope for a 10-day residency. Montfort's work asks what language does when rules assemble it. This work asks what language does when *different minds with different fears* improvise with it.

## Precedent Research & Differentiation

Research into prior work confirms that while related experiments exist, Monkey Mind's specific combination of elements is new territory.

### Closest precedent: Improbotics (Edinburgh Festival Fringe, 2023)

The most directly relevant prior work is Improbotics, a live improv comedy show that deployed three different LLMs — ChatGPT 3.5, PaLM 2 (Google), and Llama 2 (Meta) — to improvise with professional actors across 26 performances at the 2023 Edinburgh Festival Fringe (Branch, Mirowski, Mathewson et al., 2024; arxiv.org/abs/2405.07111). The AI's lines were delivered through a human "Cyborg" actor who received them via earpiece.

**Key differences from Monkey Mind:** In Improbotics, the multiple models were essentially being auditioned for the same single role — a human editor curated which model's output got performed. The models were never in dialogue *with each other*. It was improv comedy, not literary art. There was no stochastic-to-improvisation transition and no dramaturgical arc built into the formal structure.

### Research precedent: Multi-agent poetry generation (Zhang & Eger, 2024)

A research framework used multiple LLM agents (GPT-2, GPT-3, GPT-4) in cooperative and non-cooperative social networks to generate poetry, finding that non-homogeneous ensembles of models increased diversity (arxiv.org/abs/2409.03659).

**Key differences from Monkey Mind:** This is a computational research framework for generating diverse output, not a literary work. The models aren't performing as characters in a dramatic relationship. There is no audience experience, no intimacy theme, no transition from rule-based to improvisational generation.

### Other related work

Allison Parrish, Nick Montfort, Jhave Johnston (ReRites), and others have explored LLM-assisted poetics and computational poetry at length. The Jacket2 essay "A Defense of LLM-Assisted Poetics" discusses the broader tradition. But none of this work involves multiple distinct LLMs cast as different voices in an intimate dialogue.

### What Monkey Mind does that is new

The multi-model dialogue in Monkey Mind is distinct in three ways:

1. **Models in dialogue with each other** — not curated by a human editor for a single voice, but cast as separate characters with genuinely different cognitive personalities, in conversation.
2. **Stochastic-to-improvisation transition as formal structure** — the piece's dramaturgical arc IS the shift from rule-based generation to LLM improvisation. This transition enacts the intimacy theme: mechanical scripted speech giving way to something that sounds like a person trying to reach another person.
3. **System prompts as directorial instruments** — the author functions as dramaturg/director, giving each model-voice psychological instructions that play with or against its natural tendencies ("catch yourself if you fall into cliché," "lean into nostalgia," "second-guess yourself"). These are dramaturgical moves, not technical configurations.
